...
function InfoRow({ label, value, onCopy, copied, highlight }: InfoRowProps) {
  return (
    <div className={`p-4 rounded-xl flex justify-between items-center ${highlight ? 'bg-cyan-500/10 border border-cyan-500/30' : 'bg-white/5'
      }`}>
      <div>
        <p className="text-gray-400 text-sm">{label}</p>
        <p className={`text-lg font-semibold ${highlight ? 'text-cyan-400' : 'text-white'}`}>
          {value}
        </p>
      </div>
      {onCopy && (
